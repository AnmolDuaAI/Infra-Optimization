...
def calculate_cost(fcs, scs, ds_matrix, data):
    # print (np.sum(fcs))
    # print (np.sum(scs))
    # print (np.sum(ds_matrix[30,:]))
    # exit()
    # return 
    cost_cd = 0
    for i in range(4096):
        for j in range(100):
            cost_cd += data.Dist_ij[i,j] * ds_matrix[i,j]
    
    cost_if = 0
    for i in range(100):
        cost_if += (scs[i] + 1.5 * fcs[i])
    
    # Assuming max cost
    cost_dm = 50

    overall_cost = cost_cd + 25 * cost_dm + 600 * cost_if
    leaderboard_score = max(10, 100-(90*overall_cost/15000000))

    print ("Cost CD : " + str(cost_cd))
    print ("Cost IF : " + str(cost_if))
    print ("Overall : " + str(overall_cost))
    print ("Leaderboard : " + str(leaderboard_score-20))
...
def read_csv(path):
    d = pd.read_csv(path)
    year_l = d["year"].values
    data_type = d["data_type"].values
    demand_point_index = d["demand_point_index"].values
    supply_point_index = d["supply_point_index"].values
    value = d["value"].values

    indexes = np.where(year_l == 2019)[0]
    year = year_l[indexes]
    data_type = data_type[indexes]
    demand_point_index = demand_point_index[indexes]
    supply_point_index = supply_point_index[indexes]
    value = value[indexes]

    scs = value[data_type == "SCS"]
    fcs = value[data_type == "FCS"]
    ds_flattened = value[data_type == "DS"]
    ds_matrix = ds_flattened.reshape((4096,100))    
    return fcs, scs, ds_matrix

# data = GetData()
# fcs, scs, ds_matrix = read_csv("./Files/sample_submission.csv")
# calculate_cost(fcs, scs, ds_matrix, data)
# fcs, scs, ds_matrix = read_csv("./submission.csv")
# calculate_cost(fcs, scs, ds_matrix, data)